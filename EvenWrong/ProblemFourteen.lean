...
-- relied on it are affected below: one is replaced by an explicit `nlinarith`
-- certificate, the other is `sorry`d (which `EvenWrong` permits) until `SOS` is
-- added to the lakefile.
  ...
  · -- `r = 0` base case
    ...
    have hLD0 : (↑(0:ℕ) + 1) / (↑(0:ℕ) + ↑b + 2) * ((0:ℝ) + 1)
      ...
      rw [show (1 + c * 1 - ((b:ℝ) + 1)) ^ 2 / (K * 1)
        ...
        field_simp; ring]
      apply div_nonneg
      · -- was `sos`; the certificate is `12·goal = (6b + (K-4c+1))² - (K²-8Kc+2K+4c²+16c+13)`
...
theorem hard_G_nonneg_6_4 {s w : ℝ} (hs : 1 ≤ s) :
  ...
  sorry -- was `sos` (one degree-8 bivariate solve); restore once `SOS` is a dependency
